function [posterior, out] = fit_OEIRS(ID, dt, DISP, Q0)
...
save('+models/+OEIRS/param_info_OEIRS.mat', 'theta', 'phi')

%% set options

% provide dimensions
dim = struct( ...
    'n',        9, ... number of hidden states (1-4: values)
...
options.priors.muX0 = [Q0 * ones(4,1); zeros(4,1); 0];
options.priors.SigmaX0 = 0.000001 * eye(9);
...
[posterior, out] = VBA_NLStateSpaceModel(y, u, @f_OEIRS, @g_OEIRS, dim, options);
